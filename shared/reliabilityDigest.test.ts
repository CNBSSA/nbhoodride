...
// 2026-09-13: the morning message read "🟢 everything healthy" and, two
// lines below it, "Build: FAILING (exit ?) · Tests: FAILING (exit ?)". Both
// came from the same missing value: the success path wrote the analysis
// without the evidence bundle, so every fact parsed as null — which the body
// rendered as FAILING and the verdict skipped over as fine.
describe("a report whose numbers are missing", () => {
  const analysisOnly = `# PG Ride Daily Reliability Report — 2026-09-13

Everything looks fine to me.

---
_Generated by the durable Daily Reliability Agent._
`;

  it("reads the facts as unknown rather than as zero", () => {
    const f = readReport(analysisOnly);
    expect(f.checkExit).toBeNull();
    expect(f.testExit).toBeNull();
    expect(f.ready).toBeNull();
  });

  it("never calls a report it cannot read healthy", () => {
    const v = verdictOf(readReport(analysisOnly));
    expect(v.emoji).toBe("🟡");
    expect(v.text).toContain("cannot tell");
  });

  it("says not recorded, not FAILING, and never contradicts its own verdict", () => {
    const text = buildReliabilityDigest({ report: analysisOnly });
    expect(text).toContain("Build: not recorded");
    expect(text).toContain("Tests: not recorded");
    expect(text).not.toContain("FAILING");
    expect(text).not.toContain("everything healthy");
  });

  it("still reports a genuine failure as red", () => {
    expect(verdictOf(readReport(broken))).toMatchObject({ emoji: "🔴", bad: true });
  });

  it("reads a negative exit code, which is a command that never ran", () => {
    const spawnFailed = healthy.replace("npm run check exit code: 0 (clean)", "npm run check exit code: -1 (FAILING)");
    const f = readReport(spawnFailed);
    expect(f.checkExit).toBe("-1");
    expect(verdictOf(f)).toMatchObject({ emoji: "🔴", bad: true });
    expect(buildReliabilityDigest({ report: spawnFailed })).toContain("FAILING (exit -1)");
  });
});
