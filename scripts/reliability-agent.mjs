...
async function main() {
  ...
  // The evidence goes in on THIS path too, not just the degraded one. The
  // morning digest reads its facts (exit codes, /health/ready) out of the
  // report text, and this path used to write the analysis alone — so on
  // every run where the API succeeded there was nothing to read, and the
  // digest printed "Build: FAILING (exit ?)" beside "everything healthy".
  // It also means the issue comment shows the numbers the analysis is based
  // on, which is what makes it checkable.
  writeFileSync(OUT, `${reportBody}\n\n---\n\n<details>\n<summary>Evidence this analysis was based on</summary>\n\n${evidence}\n\n</details>${footer}`);
