...
export async function run({ base, db }) {
  ...
  const seed = async ({ method = "card", status = "completed", daysAgo = 0, tip = "0.00" } = {}) => {
    const { rows: [r] } = await db.query(
      `INSERT INTO rides (rider_id, driver_id, status, pickup_location, destination_location, estimated_fare, actual_fare, payment_method, payment_status,
                          platform_fee, driver_earnings, tip_amount, started_at, completed_at)
       VALUES ($1, $2, $3, $4, $5, '23.21', '23.21', $6, $7, '3.48', '19.73', $8,
               NOW() - ($9 || ' days')::interval - interval '30 minutes', NOW() - ($9 || ' days')::interval)
       RETURNING id`,
      [FIXTURES.rider.id, FIXTURES.driver.id, status, loc(PICKUP), loc(DEST), method, method === "card" ? "paid_card" : "paid_cash", tip, String(daysAgo)]);
    return r.id;
  };
