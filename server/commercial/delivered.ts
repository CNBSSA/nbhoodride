...
export async function retireOldProofPhotos(now: Date = new Date(), days: number = PROOF_PHOTO_RETENTION_DAYS): Promise<{ retired: number }> {
  ...
  const rows = await db.select({ id: commercialJobs.id, jobNumber: commercialJobs.jobNumber, proof: commercialJobs.proof }).from(commercialJobs)
    .where(and(isNotNull(commercialJobs.proof), sql`${commercialJobs.proof}->>'photoUrl' IS NOT NULL`, sql`COALESCE(${commercialJobs.proof}->>'photoRetired', 'false') <> 'true'`, sql`(${commercialJobs.proof}->>'signedAt')::timestamptz < ${cutoff}::timestamptz`));
  let retired = 0;
  for (const row of rows) {
    ...
    try {
      await db.transaction(async (tx) => {
        if (m) await tx.delete(storedObjects).where(eq(storedObjects.id, m[1]));
